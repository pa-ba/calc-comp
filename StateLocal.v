...
| vm_fail k s : ⟨ FAIL, k, s⟩ ==> ⟪k⟫
| vm_mark c h k s : ⟨MARK h c, k, s⟩ ==> ⟨c, HAN h s :: k, s⟩
| vm_unmark c n h k s s' : ⟨UNMARK c, VAL n :: HAN h s' :: k, s⟩ ==> ⟨c, VAL n :: k, s⟩
| vm_load c k s : ⟨LOAD c, k, s⟩ ==> ⟨c, VAL s :: k, s⟩
| vm_save c n k s : ⟨SAVE c, VAL n :: k, s⟩ ==> ⟨c, k, n⟩
| vm_fail_val n k : ⟪VAL n :: k ⟫ ==> ⟪k⟫
| vm_fail_han c k s : ⟪HAN c s :: k ⟫ ==> ⟨c, k, s⟩
where "x ==> y" := (VM x y).
...
Proof.
  ...
  begin
   ...
   <<= { apply vm_fail_han }
    (match eval e1 s with
         | Some (m, s') => ⟨ c, VAL m :: k, s'⟩
         | None => ⟪ HAN (comp' e2 c) s :: k⟫
    end).
   <<= { apply vm_unmark }
    (match eval e1 s with
         | Some (m, s') => ⟨ UNMARK c, VAL m :: HAN (comp' e2 c) s :: k, s'⟩
         | None => ⟪ HAN (comp' e2 c) s :: k⟫
    end).
   <<= { apply IHe1 }
       ⟨ comp' e1 (UNMARK c), HAN (comp' e2 c) s :: k, s⟩.
   <<= { apply vm_mark }
